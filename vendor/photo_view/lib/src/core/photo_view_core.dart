...
class PhotoViewCoreState extends State<PhotoViewCore>
  ...
  void onScaleStart(ScaleStartDetails details) {
    ...
    _positionBefore = controller.position;
  ...
  void onScaleUpdate(ScaleUpdateDetails details) {
    ...
    // PATCHED: 以手势焦点(双指中点)为缩放锚点——保持手势开始时位于
    // 焦点下方的图像点始终跟随当前焦点,而不是绕图片中心缩放。
    // 变换为 y = position + scale * x(x/y 均以视口中心为原点)。
    final Offset viewportCenter =
        scaleBoundaries.outerSize.center(Offset.zero);
    final Offset focal = details.localFocalPoint - viewportCenter;
    final Offset focalBefore = _startLocalFocalPoint! - viewportCenter;
    final Offset newPosition =
        focal - (focalBefore - _positionBefore!) * details.scale;
    ...
    updateMultiple(
      ...
      position: widget.enablePanAlways
          ? newPosition
          : clampPosition(position: newPosition, scale: newScale),
      rotation:
  ...
  // PATCHED: 双击定点缩放(仿微信)。
  // 未放大时:以双击点为锚点放大到 max(覆盖屏幕, 2 倍初始);
  // 已放大时:回到初始大小并居中。
  ...
  void onDoubleTap() {
    final double initialScale = scaleBoundaries.initialScale;
    final double currentScale = scale;

    if (currentScale > initialScale * 1.01) {
      scaleStateController.setInvisibly(PhotoViewScaleState.initial);
      animateScale(currentScale, initialScale);
      animatePosition(controller.position, Offset.zero);
      return;
    }

    final double zoomScale = math
        .max(
          getScaleForScaleState(PhotoViewScaleState.covering, scaleBoundaries),
          initialScale * 2.0,
        )
        .clamp(scaleBoundaries.minScale, scaleBoundaries.maxScale);
    if (zoomScale <= currentScale * 1.01) {
      return;
    }

    // 保持双击点下方的图像点在缩放后仍位于双击点:P' = T - (T - P0) * (s'/s0)
    final Offset viewportCenter =
        scaleBoundaries.outerSize.center(Offset.zero);
    final Offset tap =
        (_doubleTapLocalPosition ?? viewportCenter) - viewportCenter;
    final Offset targetPosition = clampPosition(
      position: tap - (tap - controller.position) * (zoomScale / currentScale),
      scale: zoomScale,
    );

    scaleStateController.setInvisibly(PhotoViewScaleState.zoomedIn);
    animateScale(currentScale, zoomScale);
    animatePosition(controller.position, targetPosition);
  }
