...
// [PATCH] dispose 真正销毁 WebView 所需的延迟销毁请求,见 PATCHES.md。
typedef struct {
  WebviewAllLinuxPlugin *plugin;
  gint id;
} PendingWebViewDispose;

gboolean dispose_webview_idle_cb(gpointer user_data) {
  PendingWebViewDispose *pending =
      static_cast<PendingWebViewDispose *>(user_data);
  // 从表里摘掉即触发 destroy_linux_webview:gtk_widget_destroy + 各种 unref,
  // WebKitWebView 随之释放,它的 WebKitWebProcess 也才会退出。
  g_hash_table_remove(pending->plugin->webviews, GINT_TO_POINTER(pending->id));
  update_flutter_view_input_region(pending->plugin);
  g_free(pending);
...
void instance_method_call_cb(FlMethodChannel *channel,
  ...
  if (strcmp(method, "dispose") == 0) {
    // [PATCH] 上游这里只 gtk_widget_hide,WebView 一直留在 plugin->webviews 里,
    // 活到插件销毁(即进程退出)为止 —— 每个 WebKitWebView 背后是一个常驻的
    // WebKitWebProcess,工作台反复开关页签会无限堆积。见 PATCHES.md。
    gtk_widget_hide(GTK_WIDGET(webview->web_view));
    webview->visible = FALSE;
    respond(method_call, success_response());

    // 真正的销毁挪到 idle:destroy_linux_webview 会 unref 掉
    // webview->method_channel,而此刻正在这个 channel 的方法回调里,就地销毁
    // 是 use-after-free。回调返回后主循环再做。
    PendingWebViewDispose *pending = g_new0(PendingWebViewDispose, 1);
    pending->plugin = webview->plugin;
    pending->id = webview->id;
    g_idle_add(dispose_webview_idle_cb, pending);
    return;
  }
