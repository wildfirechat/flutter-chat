...
struct _WindowManagerPlugin {
  ...
  // [PATCH] 多窗口场景的生命周期清理,见 vendor/window_manager/PATCHES.md。
  // 插件注册时挂到本窗口上的信号回调、以及挂到 GtkWidget 类型上的全局
  // button-press-event emission hook,都以 plugin 自身作为 user_data。
  // 子窗口引擎关闭时 plugin 会被释放,若不摘掉这些回调就是 use-after-free。
  GtkWindow* window;            // 弱指针,窗口先销毁时自动置空
  gulong mouse_press_hook_id;   // g_signal_add_emission_hook 返回的 hook id
};
...
static FlMethodResponse* pop_up_window_menu(WindowManagerPlugin* self) {
  ...
  // [PATCH] seat 上没有指针设备时(GdkSeat 尚未就绪 / 无鼠标),原实现直接把 NULL
  // 传给 gdk_device_get_position,报 assertion 'GDK_IS_DEVICE (device)' failed,
  // 还会把 NULL device 塞进伪造的 GdkEvent 里。这里直接放弃本次调用。
  if (window == nullptr || pointer == nullptr) {
    g_warning("window_manager: 没有可用的指针设备,popUpWindowMenu 跳过");
    return FL_METHOD_RESPONSE(
...
static FlMethodResponse* start_dragging(WindowManagerPlugin* self) {
  ...
  // [PATCH] 同 pop_up_window_menu:窗口已销毁或 seat 上没有指针设备时直接返回,
  // 否则 gdk_device_get_position 会报 'GDK_IS_DEVICE (device)' failed,
  // 并用未初始化的坐标去 begin_move_drag。
  if (window == nullptr) {
    return FL_METHOD_RESPONSE(
        fl_method_success_response_new(fl_value_new_bool(false)));
  }
  ...
  auto seat = display != nullptr ? gdk_display_get_default_seat(display)
                                 : nullptr;
  auto device = seat != nullptr ? gdk_seat_get_pointer(seat) : nullptr;
  if (device == nullptr) {
    g_warning("window_manager: 没有可用的指针设备,startDragging 跳过");
    return FL_METHOD_RESPONSE(
        fl_method_success_response_new(fl_value_new_bool(false)));
  }
...
static FlMethodResponse* start_resizing(WindowManagerPlugin* self,
  ...
  auto device = seat != nullptr ? gdk_seat_get_pointer(seat) : nullptr;
  if (device == nullptr) {
    g_warning("window_manager: 没有可用的指针设备,startResizing 跳过");
    return FL_METHOD_RESPONSE(
        fl_method_success_response_new(fl_value_new_bool(false)));
  }
...
static void window_manager_plugin_dispose(GObject* object) {
  ...
  // [PATCH] 摘掉所有仍持有 self 的回调,否则子窗口关闭(引擎销毁 -> plugin
  // 释放)后,主窗口里的下一次鼠标点击就会以已释放的指针进入 on_mouse_press,
  // 并 memcpy 覆写这块内存,表现为
  // "invalid cast from 'GtkCssStaticStyle' to 'WindowManagerPlugin'" 后崩溃。
  if (self->mouse_press_hook_id != 0) {
    g_signal_remove_emission_hook(
        g_signal_lookup("button-press-event", GTK_TYPE_WIDGET),
        self->mouse_press_hook_id);
    self->mouse_press_hook_id = 0;
  }
  if (self->window != nullptr) {
    g_signal_handlers_disconnect_matched(self->window, G_SIGNAL_MATCH_DATA, 0,
                                         0, nullptr, nullptr, self);
    g_object_remove_weak_pointer(G_OBJECT(self->window),
                                 reinterpret_cast<gpointer*>(&self->window));
    self->window = nullptr;
  }
  self->_event_box = nullptr;

  g_clear_object(&self->css_provider);
  // [PATCH] 注册时持有的引用在这里释放(原实现两者都泄漏)。
  g_clear_object(&self->channel);
  g_clear_object(&self->registrar);
  g_free(self->title_bar_style_);
  self->title_bar_style_ = nullptr;
...
void window_manager_plugin_register_with_registrar(
  ...
  // [PATCH] 记住信号所挂的窗口,供 dispose 时断开;用弱指针以应对窗口先于
  // plugin 销毁的情况(子窗口 gtk_widget_destroy 会先干掉窗口)。
  plugin->window = get_window(plugin);
  if (plugin->window != nullptr) {
    g_object_add_weak_pointer(G_OBJECT(plugin->window),
                              reinterpret_cast<gpointer*>(&plugin->window));
  }

  // [PATCH] emission hook 是挂在 GtkWidget 类型上的全局钩子,进程内所有窗口的
  // 点击都会命中它;必须记下 id,在 dispose 时移除。
  plugin->mouse_press_hook_id = g_signal_add_emission_hook(
