...
void Webview::SetJavaScriptDialogCallbacksEnabled(bool alert, bool confirm,
  ...
  // [PATCH] WebView2 的 ScriptDialogOpening 事件**只在默认脚本对话框被关掉时**
  // 才会触发(见 ICoreWebView2::add_ScriptDialogOpening 文档)。上游注册了
  // handler 却从没设过这个开关,于是 alert/confirm/prompt 一律由 WebView2 自己
  // 弹原生框,Dart 侧的回调永远收不到 —— 而 dsbridge 的 JS→Dart 通道正是靠拦
  // prompt() 实现的,工作台里点任何东西都没反应。见 PATCHES.md。
  if (settings_) {
    settings_->put_AreDefaultScriptDialogsEnabled(
        (alert || confirm || prompt) ? FALSE : TRUE);
  }
}
