...
class MessageInputBarController extends ChangeNotifier {
  ...
  Future<void> sendSticker(String stickerPath) async {
    try {
      final sticker = await StickerOutbox.prepare(stickerPath);
      if (sticker.needsUpload) {
        conversationViewModel.sendMediaMessage(sticker.content,
            uploadedCallback: sticker.rememberRemoteUrl);
      } else {
        conversationViewModel.sendMessage(sticker.content);
      }
      onSend?.call();
    } catch (e) {
