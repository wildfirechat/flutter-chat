...
/// 桌面形态输入栏:工具条(表情/图片/文件)+ 多行输入区 + 发送按钮。
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                // 通话入口靠右(微信 PC 布局);群通话暂未实现,仅单聊显示
                if (controller.conversation.conversationType == ConversationType.Single) ...[
                  _ToolbarButton(
                    icon: Icons.call_outlined,
                    tooltip: l10n.audioCallAction,
                    onTap: () => startSingleAvCall(context, controller.conversation.target, audioOnly: true),
                  ),
                  _ToolbarButton(
                    icon: Icons.videocam_outlined,
                    tooltip: l10n.videoCallAction,
                    onTap: () => startSingleAvCall(context, controller.conversation.target, audioOnly: false),
                  ),
                ],
              ],
            ),
        ...
        builder: (context, hovered) => GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: hovered ? Colors.black.withValues(alpha: 0.06) : Colors.transparent,
              borderRadius: BorderRadius.circular(4),
            ),
            child: Icon(icon, size: 21, color: const Color(0xFF5C5C5C)),
          ),
        ),
      ),
    );
