...
public class PushService {
    ...
    /**
     * 推送通道编号，必须与推送服务端 {@code AndroidPushType} 以及 IM 服务端
     * {@code t_user_session._push_type} 保持一致，不要随意改动。
     * 魅族(3) 推送服务端已移除支持，本模块不再集成。
     */
    ...
    public static void init(Application gContext, String applicationId) {
        ...
        try {
            ...
            Log.d(TAG, "push service type " + INST.pushServiceType);
    ...
    private static boolean useGoogleFCM(Context context) {
        ...
        String no = tm == null ? null : tm.getNetworkOperator();
        String simNo = tm == null ? null : tm.getSimOperator();

        if (!TextUtils.isEmpty(no) && !TextUtils.isEmpty(simNo)) {
            //当sim卡和网络都是大陆运营商时，不能使用fcm；有一个不是大陆运营商时，可以使用fcm
            return !(no.startsWith("460") && simNo.startsWith("460"));
        }

        //取不到运营商信息（如无 SIM 卡）时退化为按区域判断：
    ...
    private boolean initXiaomi(Context context) {
        ...
        try {
            ...
            if (appInfo.metaData != null) {
                // 不能用 getString：纯数字的 meta-data 若能放进 int32 会被 aapt 编成整型，
                // 此时 getString 返回 null，小米推送会静默注册失败。
                String appid = metaDataAsString(appInfo, "MIPUSH_APPID");
                String appkey = metaDataAsString(appInfo, "MIPUSH_APPKEY");
    ...
    /**
     * 读 manifest meta-data 并统一转成字符串。meta-data 的类型由 aapt 按字面量推断，
     * 纯数字会变成整型，直接 getString 会拿到 null。
     */
    private static String metaDataAsString(ApplicationInfo appInfo, String key) {
    ...
    private void initVIVO(Context context) {
        ...
        PushClient.getInstance(context).turnOnPush(new IPushActionListener() {
            ...
            public void onStateChanged(int state) {
                Log.d(TAG, "vivo turnOnPush " + state);
